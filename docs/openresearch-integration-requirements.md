...
## 10. Beyond interview: other domains (Stock, Real Estate, Finance, personal) — not built, but the routing decision is made

A natural next question once Pluely can reach OpenResearch at all: could the same integration also answer Stock, Real Estate, or general Finance questions — or even something like personal/background information — not just interview coaching? Settled as a direction, **not implemented**:

**These stay single-tenant local stores, unlike interview memory.** §6's candidate_id-scoped, markdown+vector+graph cognitive memory is specific to the interview-coaching use case — it exists because a candidate's strengths/weaknesses need to persist and compound across sessions for *that one person*. Stock (watchlist, portfolio), Real Estate, and general Finance don't have that same cross-session-per-person requirement in this codebase today, and there's no product need yet to bolt candidate_id scoping onto them. They stay exactly as they are — single-tenant local stores/pipelines (`store/watchlist_store.py`, `store/portfolio_store.py`, `pipelines/realestate_pipeline.py`, etc.) — rather than being generalized into a second cognitive-memory system prematurely.

**Domain routing is explicit, not inferred.** The real question this raises is: if a client sends a question, how does OpenResearch know whether it's an interview-coaching question, a stock question, a real-estate question, or something else? The decision: **the client says so by which endpoint it calls — domain is never guessed from the free-text question itself.**

- This repo already has a working example of the alternative (LLM-based intent classification) in `agents/stock/query_router.py`'s `QueryRouterAgent`, which classifies free text into stock-specific intents (`single_analysis`/`comparison`/`watchlist_add`/`unknown`) via an LLM call with a regex fallback. That's an acceptable design *within* one domain, where a wrong guess just costs a clarification round-trip.
- It is **not** an acceptable design *across* domains for a live interview session. A misclassification there means the candidate asked a real interview question mid-conversation and got a stock or real-estate answer back instead — a much worse failure mode, with no clean in-the-moment recovery, than the existing single-domain router's "ask for clarification" fallback.
- Pluely (or any client) already has this information for free: whichever feature/UI surface the user is in *is* the domain — the Interview Assistant screen calling `/v1/interview/answer` isn't inferring anything, it's just the feature the candidate opened. The same would hold for a Stock or Real Estate mode.

**What this implies for future endpoints, if/when built:** each domain gets its own endpoint, mirroring `/v1/interview/answer`'s shape but without candidate_id scoping — e.g. `POST /v1/stock/answer`, `POST /v1/realestate/answer`, `POST /v1/finance/answer` — each backed by the existing single-tenant pipeline for that domain (`pipelines/stock_pipeline.py`, `pipelines/realestate_pipeline.py`, etc.), the same way `agents/interview/skills/live_interview_coach.py` wraps the interview memory today. Routing is then just "which URL the client called," with zero cross-domain classification risk.

**Left open, deliberately, for when this becomes real work**: an LLM-based cross-domain router (a top-level version of `QueryRouterAgent`) could still be added *later*, as a fallback for a single unified "just ask anything" entry point, if that ever becomes an actual product requirement — but it would be additive to explicit per-domain endpoints, not a replacement for them, and specifically excluded from the interview-coaching path given the misclassification risk above.
