...
## 10. Beyond interview: other domains (Stock, Real Estate, Finance, personal)

A natural next question once Pluely can reach OpenResearch at all: could the same integration also answer Stock, Real Estate, or general Finance questions — or even something like personal/background information — not just interview coaching? The routing decision and Stock/Real-Estate endpoints are **implemented**; Finance and "personal" are **not**, deliberately — see below for exactly which is which.

**These stay single-tenant local stores, unlike interview memory.** §6's candidate_id-scoped, markdown+vector+graph cognitive memory is specific to the interview-coaching use case — it exists because a candidate's strengths/weaknesses need to persist and compound across sessions for *that one person*. Stock (watchlist, portfolio), Real Estate, and general Finance don't have that same cross-session-per-person requirement in this codebase today, and there's no product need yet to bolt candidate_id scoping onto them. **Implemented as designed**: they stay exactly as they were — single-tenant local stores/pipelines (`store/watchlist_store.py`, `store/portfolio_store.py`, `pipelines/realestate_pipeline.py`, etc.) — nothing was generalized into a second cognitive-memory system.

**Domain routing is explicit, not inferred — implemented.** If a client sends a question, how does OpenResearch know whether it's an interview-coaching question, a stock question, a real-estate question, or something else? **The client says so by which endpoint it calls — domain is never guessed from the free-text question itself.** No cross-domain classifier exists or is planned as the default mechanism.

- This repo already had a working example of the alternative (LLM-based intent classification) in `agents/stock/query_router.py`'s `QueryRouterAgent`, which classifies free text into stock-specific intents (`single_analysis`/`comparison`/`watchlist_add`/`unknown`) via an LLM call with a regex fallback. That's an acceptable design *within* one domain, where a wrong guess just costs a clarification round-trip — and it's exactly what §10's Stock endpoint reuses (below), rather than duplicating.
- It would **not** be an acceptable design *across* domains for a live interview session. A misclassification there means the candidate asked a real interview question mid-conversation and got a stock or real-estate answer back instead — a much worse failure mode, with no clean in-the-moment recovery, than the existing single-domain router's "ask for clarification" fallback. This is why no top-level cross-domain classifier was built.
- Pluely (or any client) already has this information for free: whichever feature/UI surface the user is in *is* the domain — the Interview Assistant screen calling `/v1/interview/answer` isn't inferring anything, it's just the feature the candidate opened. The same holds for the Stock/Real Estate endpoints below.

### 10.1 Implementation status — built

| Domain | Status | Endpoint | Backed by |
|---|---|---|---|
| Stock | **Implemented** | `POST /v1/stock/answer` | `agents/stock/stock_answer_skill.py` (`StockAnswerSkill`) wrapping the existing `QueryRouterAgent` + `StockResearchPipeline` |
| Real Estate | **Implemented** | `POST /v1/realestate/answer` | `agents/realestate/realestate_answer_skill.py` (`RealEstateAnswerSkill`) wrapping `RealEstatePipeline` |
| Finance | **Not implemented** | — | No dedicated finance pipeline exists in this codebase to wrap — see below |
| Personal/background info | **Not implemented** | — | Never had a concrete design beyond the original framing question — see below |

Both implemented endpoints mirror `/v1/interview/answer`'s wire shape (`schemas/domain_answer.py`'s `DomainAnswerRequest`/`DomainAnswerResult`) — SSE streaming, `answer_chunk`/`done`/`metadata.matched_sources`/`metadata.images_ignored` — but **no `candidate_id`**, matching the single-tenant decision above. `AnswerStyle`, `ImageAttachment`, and `MatchedSource` were factored out of `schemas/interview_memory.py` into `schemas/answer_common.py` so these two schemas didn't have to import an interview-specific module for pieces that were never actually interview-specific; `interview_memory.py` re-exports them, so nothing that already imported from there broke.

**`StockAnswerSkill`** reuses `QueryRouterAgent` as-is for intent/ticker extraction (no second NLU layer built) and handles all four of its intents: `single_analysis` (research one ticker, answer grounded in the resulting brief), `comparison` (researches both tickers, answers grounded in both), `watchlist_add` (no LLM synthesis call needed — just calls `WatchlistStore.add()` and confirms), and the router's own clarification short-circuit (no research run at all if the question doesn't resolve to a company).

**`RealEstateAnswerSkill`** needed one new piece `StockAnswerSkill` didn't: `RealEstatePipeline.run()` takes structured `city`/`state` (+ optional property details), not free text — there was no real-estate equivalent of `QueryRouterAgent`'s extraction step. Added `schemas/realestate.py`'s `RealEstateQueryExtraction` and a small LLM call that pulls city/state/property details out of the question before the pipeline runs; if city or state can't be extracted, the skill asks for clarification rather than guessing or failing.

**Verified live, end-to-end**, against the actual configured LLM backend (LM Studio): `POST /v1/stock/answer` with `{"question": "Is AAPL a good buy right now?"}` ran the full pipeline (data fetch → fundamentals → sentiment → synthesis → backtest) and produced a specific, correctly-grounded streamed answer citing real figures from that run (price, fair-value range, P/E, RSI, backtest return) — not a generic response. Round-trip took roughly 100 seconds end-to-end on a local model at `depth="full"` — noticeably slower than §2's ~2s interview-answer target, because this chains the *entire* multi-node research pipeline plus one more grounding LLM call, not a single retrieval-and-answer step. **This latency is not yet addressed** — no "quick mode" is exposed on `DomainAnswerRequest`, and `QueryRouterAgent`'s own depth classification (not the caller) currently decides `quick` vs `full`. Worth revisiting if these endpoints need to feel as responsive as the interview one.

**Finance — deliberately not built.** Unlike Stock and Real Estate, there's no dedicated finance pipeline anywhere in this codebase to wrap. Building `/v1/finance/answer` would have meant inventing pipeline behavior from scratch rather than following the "wrap an existing single-tenant pipeline" pattern the other two follow — that's new, undesigned scope, not a missing wire-up. No endpoint, no skill, no schema exist for it.

**"Personal information" — deliberately not built.** This was named in the original framing question but never got a concrete design: it's unclear whether it means a general user-profile domain, an extension of interview's résumé/JD data to non-interview contexts, or something else — worth a real product conversation before anything gets built here, not an assumption.

**Tests**: `tests/test_domain_answer_skills.py` — 7 tests, mocked LLM/pipeline dependencies, covering clarification short-circuiting, watchlist-add bypassing the LLM entirely, single-ticker and comparison research grounding (asserting the actual research content reaches the prompt, not just a generic question), and image-ignored propagation.

**Left open, deliberately, for when this becomes real work**: an LLM-based cross-domain router (a top-level version of `QueryRouterAgent`) could still be added *later*, as a fallback for a single unified "just ask anything" entry point, if that ever becomes an actual product requirement — but it would be additive to the explicit per-domain endpoints above, not a replacement for them, and specifically excluded from the interview-coaching path given the misclassification risk described above.
